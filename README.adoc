...
For example, to add musl repository, run:
 ...
 # docker exec -it elixir-container /usr/local/elixir/utils/index-repository musl https://git.musl-libc.org/git/musl

Or, to run indexing in a separate container:

 # docker run -v ./elixir-data/:/srv/elixir-data --entrypoint /usr/local/elixir/utils/index-repository elixir musl https://git.musl-libc.org/git/musl
...
Error logs are available in `/var/log/apache2/error.log` and `/tmp/elixir-errors` within the container.
